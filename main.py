...
#obstacles that will kill the dots if they touch it
obstacles = []
class Obstacle:
...
#checkpoint that gives bonus to fitness of any dot that reaches it
checkpoints = []
class Checkpoint:
    def __init__(self, x, y, height, width):
        self.x = x
...
def main():
    ...
    while dots_are_alive:
        ...
        #draw dots
        for i in range(len(population.dots_list)-1,0,-1): #iterates through each dot
            ...
            for check in range(len(checkpoints)): #check if they got a checkpoint
                if checkpoints[check].check_got(dot.x,dot.y) and not check in dot.checks_reached:#dont let them get credit for the same chackpoint multiple times
                    dot.checks_reached.append(check)
            ...
            if not dot.dead:
                if not dot.check_goal(goal.x, goal.y,goal.radius):
...
while running: #runs once per generation
    ...
    text = font.render(('Gen ' +str(gen)), True, blue, black) #updates gen counter in corner
